...
path_of_exile_location = {}
path_of_exile_location.__index = path_of_exile_location
...
function path_of_exile_location.new(name)
    local self = setmetatable({}, path_of_exile_location)
    if name then
...
function path_of_exile_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        exit = path_of_exile_location.new(exit)
    end
    if rule == nil then
        rule = always
    end
...
function path_of_exile_location:connect_two_ways(exit, rule)
...
function path_of_exile_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function path_of_exile_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function path_of_exile_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function path_of_exile_location:accessibility()
...
-- 
function path_of_exile_location:discover(accessibility, keys)
...
entry_point = path_of_exile_location.new("entry_point")
